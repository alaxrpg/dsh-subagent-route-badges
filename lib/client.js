window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function routeBadge(value, quiet = false) {
			if (!value || typeof value !== 'object') return quiet ? null : '路由未知';
			const provider = typeof value.provider === 'string' ? value.provider : null;
			const model = typeof value.model === 'string' ? value.model : null;
			if (!provider || !model) return quiet ? null : '路由未知';
			const effort = typeof value.reasoningEffort === 'string' ? value.reasoningEffort : '默认';
			const text = `${provider} / ${model} · 思考 ${effort}`;
			return (0, react_jsx_runtime.jsx)("span", { className: ROUTE_BADGE_PREFIX + 'root', title: value.reasoningSource === 'adapter-default' ? `${text}（适配器默认）` : text, "aria-label": text, "data-dsh-route-badge": "true", children: text });
		}
								...
								children: [
									...
									(0, react_jsx_runtime.jsxs)("span", {
										...
										children: [(0, react_jsx_runtime.jsx)("span", {
											...
											className: SubagentHeaderLineage_module_css_default.summary,
											children: secondary
										}), routeBadge(summary?.projectionValues?.subagentRouteBadge)]
									}),
		...
		*/
		function RouteBadgeAction({ useSession }) {
			...
			if (!session?.subagent || !session.projectionValues?.subagentRouteBadge) return null;
			return routeBadge(session.projectionValues.subagentRouteBadge, true);
		}
		function apply(ctx) {
