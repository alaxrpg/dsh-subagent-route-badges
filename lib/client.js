window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		* Reads the wire projection through the standard useProjection seat —
		* the session snapshot never carries projectionValues itself. */
		function RouteBadgeAction({ useSession, useProjection }) {
			const subagent = useSession((s) => s.subagent) ?? null;
			const value = useProjection("subagentRouteBadge");
			if (subagent === null) return null;
			return routeBadge(value);
		}
		//#endregion
